...
protected: 
	object(char b,short x,short y,bool s){
		position_x = x;
		position_y = y;
		body = b;
		side = s;
...
public:
	wall(short x,short y):	object(c,x,y,s);
	char returnbody(){
		return b;
	}
	bool returnside(){
...
public:
	space(short x,short y):	object(c,x,y,s);
	char returnbody(){
		return b;
	}
	bool returnside(){
...
public:
	...
	void draw_Frame()  //畫邊框
	{
		...
		{
			gotoxy(i, 0);
			cout << char(64)<<char(124);//■保留用 
			gotoxy(i, height);
			cout << char(64)<<char(124);
		}
	}
	void init_snake()  //初始化蛇
	{
		...
		snake.push_back({ ' ',short((height / 2) + 4) }); //新增蛇尾，先放空，以便於後面新增節數時操作
	}
	void draw_Snake() //畫蛇
	{
	...
	{
		for (deque<Snake>::iterator i = snake.begin(); i != snake.end(); i++) //c++11的基於範圍的迴圈
		{
